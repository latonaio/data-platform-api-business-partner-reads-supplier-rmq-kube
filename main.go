...
import (
	dpfm_api_caller "data-platform-api-business-partner-reads-supplier-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-business-partner-reads-supplier-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncBusinessPartnerSupplierReads(accepter, &input, &output, l)
	if len(errs) != 0 {
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"Supplier", "Contact", "PartnerFunction", "PartnerFunctionContact", "PartnerPlant", "FinInst", "Tax", "Accounting",
		}
	}
	return accepter
}
